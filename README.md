<div align="center">

<!-- Animated Header Banner -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,50:1a1b27,100:0e75b6&height=220&section=header&text=Shubham%20Kumar%20Gupta&fontColor=ffffff&fontSize=42&fontAlignY=35&desc=Cloud%20Engineer%20%E2%80%A2%20Cloud%20Infrastructure%20%E2%80%A2%20Full-Stack%20Developer&descSize=18&descAlignY=55&animation=fadeIn" />

<!-- Typing SVG -->
<a href="https://git.io/typing-svg" target="_blank">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=0E75B6&center=true&vCenter=true&random=false&width=620&lines=Building+Scalable+Cloud+Infrastructure+%E2%98%81%EF%B8%8F;Automating+Deployments+with+CI%2FCD+%F0%9F%94%84;Containerizing+Apps+with+Docker+%F0%9F%90%B3;Infrastructure+as+Code+with+Terraform+%F0%9F%8F%97%EF%B8%8F;Full-Stack+Developer+%7C+React+%2B+Node.js+%F0%9F%9A%80;300%2B+DSA+Problems+Solved+%F0%9F%92%AA" alt="Typing SVG" />
</a>

<br/>

<img src="https://komarev.com/ghpvc/?username=shubham221716&label=Profile%20views&color=0e75b6&style=for-the-badge" alt="Profile Views" />
&nbsp;
<a href="https://shubh45.netlify.app/" target="_blank"><img src="https://img.shields.io/badge/Portfolio-FF5722?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Portfolio" /></a>

</div>

---

## <img src="https://media.giphy.com/media/WUlplcMpOCEmTGBtBW/giphy.gif" width="30"> About Me

```yaml
name: Shubham Kumar Gupta
role: Assistant System Engineer (Cloud)
company: Tata Consultancy Services (TCS)
location: Varanasi, Uttar Pradesh, India
education: B.Tech — CSE (KIET, AKTU) | 2020–2024

currently_learning: ["Kubernetes", "Ansible", "GitOps Workflows"]
ask_me_about: ["DevOps", "CI/CD", "AWS", "Docker", "Terraform", "React", "Node.js"]
portfolio: "https://shubh45.netlify.app"

fun_fact: "I automate things so I can be lazy efficiently 😄"
```

<img align="right" alt="DevOps Animation" width="320" src="https://user-images.githubusercontent.com/74038190/212749447-bfb7e725-6987-49d9-ae85-2015e3e7cc41.gif" />

### 💼 What I Do

- 🏢 **Cloud Engineer @ TCS** — Building production-grade infrastructure
- 🚀 **Reduced deployment time by 40%** with Jenkins & GitHub CI/CD pipelines
- ☁️ **Cut AWS provisioning from 2hrs → 15 min** using Terraform (EC2, S3, IAM, VPC)
- 🐳 **Eliminated 60% manual effort** through Docker-to-EC2 pipeline automation
- 🔧 Maintained environment consistency across **dev, staging & production**
- 🌐 Previously a **Web Developer @ Groveus Informatics** — built client-facing apps with PHP, JS & SEO optimization

### 🧠 Beyond Work

- 💡 Solved **300+ DSA problems** on LeetCode (Arrays, Strings, Stacks, Queues, Hashing)
- ⭐ **2-Star CodeChef** · Max Rating **1443** · Silver Badge · **30+ contests**
- 🌱 Currently learning **Kubernetes**, **Ansible**, and **GitOps** workflows

<br clear="right"/>

---

## 🛠️ Tech Arsenal

<div align="center">

### ☁️ DevOps & Cloud

<a href="#"><img src="https://skillicons.dev/icons?i=aws,docker,terraform,jenkins,nginx,linux,kubernetes,git,github,bash&theme=dark" alt="DevOps Skills" /></a>

### 💻 Languages & Frameworks

<a href="#"><img src="https://skillicons.dev/icons?i=js,react,nodejs,java,cpp,php,html,css,bootstrap,mongodb,mysql&theme=dark" alt="Dev Skills" /></a>

</div>

<details>
<summary><b>📋 Detailed Skill Breakdown</b></summary>
<br/>

| Category | Technologies |
|:---|:---|
| **Cloud & Infra** | AWS EC2, S3, IAM, VPC · Terraform (remote state) · Nginx Reverse Proxy |
| **CI/CD** | Jenkins Pipelines · GitHub Actions · Automated Build/Test/Deploy |
| **Containers** | Docker · Docker Compose · Image Builds · Container Orchestration |
| **Languages** | JavaScript · C/C++ · Java · PHP · SQL · HTML5 · CSS3 |
| **Frameworks** | React · Node.js · CodeIgniter · Bootstrap 5 |
| **Databases** | MongoDB · MySQL |
| **OS & Tools** | Linux · Git · GitHub · VS Code |
| **Learning** | Kubernetes · Ansible · GitOps |

</details>

---

## 💼 Experience

<table>
<tr>
<td width="120" align="center">
  <img src="https://img.shields.io/badge/TCS-0A66C2?style=for-the-badge&logo=tata&logoColor=white" alt="TCS" />
  <br/>
  <sub><b>Jun 2025 – Present</b></sub>
</td>
<td>

### Assistant System Engineer (DevOps) — TCS
📍 Varanasi, UP

- Built & optimized **CI/CD pipelines** using Jenkins & GitHub — enabled daily automated releases, **cut deployment time by 40%**
- Eliminated **60% of manual Docker-to-EC2 deployment** effort through full pipeline automation
- Provisioned AWS infra (EC2, S3, IAM, VPC) with **Terraform & remote state** — reduced provisioning from **2 hrs → 15 min**
- Maintained environment consistency across **dev, staging, and production**

</td>
</tr>
<tr>
<td width="120" align="center">
  <img src="https://img.shields.io/badge/Groveus-43853D?style=for-the-badge&logoColor=white" alt="Groveus" />
  <br/>
  <sub><b>Jul 2024 – Jun 2025</b></sub>
</td>
<td>

### Web Developer — Groveus Informatics
📍 Siliguri, WB

- Designed **responsive websites** using PHP, JavaScript, HTML/CSS with full cross-device compatibility
- Applied **SEO best practices** and performance optimizations to boost search visibility
- Built and maintained **client-facing web applications** for real business use cases

</td>
</tr>
</table>

---

## 🚀 Featured Projects

<div align="center">

<table>
<tr>
<td width="50%" valign="top">

### 🌤️ [SkyWatch — Weather Dashboard](https://skywatch45.netlify.app/)
Real-time weather companion with **5-day forecasts**, air quality data (PM2.5, PM10, O3, NO2), and interactive maps. Dark mode support.

`JavaScript` `OpenWeather API` `HTML/CSS`

[![Live Demo](https://img.shields.io/badge/🔗_Live_Demo-0e75b6?style=for-the-badge)](https://skywatch45.netlify.app/)

</td>
<td width="50%" valign="top">

### 🍕 [Crisp Corner — Food Ordering](https://crispcorner.netlify.app/)
Full-stack food ordering platform with **real-time order processing**. REST APIs handling **100+ daily requests**. Optimized MongoDB queries reducing response time by **30%**.

`React` `Node.js` `MongoDB` `REST API`

[![Live Demo](https://img.shields.io/badge/🔗_Live_Demo-0e75b6?style=for-the-badge)](https://crispcorner.netlify.app/)

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🔄 [DevOps CI/CD Pipeline](https://github.com/shubham221716/devops)
End-to-end automated pipeline using **Jenkins & GitHub** for build, test, deploy. Containerized with Docker, deployed on **AWS EC2 via Nginx** reverse proxy.

`Jenkins` `Docker` `AWS EC2` `Nginx`

[![GitHub](https://img.shields.io/badge/📂_Source_Code-181717?style=for-the-badge&logo=github)](https://github.com/shubham221716/devops)

</td>
<td width="50%" valign="top">

### 🌿 [Weed Detector — ML Project](https://github.com/shubham221716/Weed-Detection)
Automatic weed detection system for **agriculture & landscaping** using computer vision and machine learning techniques.

`Python` `Machine Learning` `Computer Vision`

[![GitHub](https://img.shields.io/badge/📂_Source_Code-181717?style=for-the-badge&logo=github)](https://github.com/shubham221716/Weed-Detection)

</td>
</tr>
<tr>
<td width="50%" valign="top" colspan="2" align="center">

### 📦 [Packers & Movers — Business Website](https://rupamcargopackers.com/)
Publicly hosted business website with **service listings, contact forms**, and fully responsive design on a PHP + CodeIgniter backend.

`PHP` `CodeIgniter` `HTML/CSS` `JavaScript`

[![Live Site](https://img.shields.io/badge/🔗_Live_Site-0e75b6?style=for-the-badge)](https://rupamcargopackers.com/)

</td>
</tr>
</table>

</div>

---

## 🎓 Education

<table>
<tr>
<td align="center" width="80">🎓</td>
<td>
  <b>B.Tech — Computer Science & Engineering</b><br/>
  KIET Group of Institutions (AKTU) · Ghaziabad, UP<br/>
  <sub>2020 – 2024 · <b>74.8%</b></sub>
</td>
</tr>
<tr>
<td align="center" width="80">📚</td>
<td>
  <b>Senior Secondary (CBSE) — Science</b><br/>
  Jawahar Navodaya Vidyalaya · Mau, UP<br/>
  <sub>2017 – 2019 · <b>91.8%</b></sub>
</td>
</tr>
<tr>
<td align="center" width="80">📖</td>
<td>
  <b>High School (CBSE)</b><br/>
  Jawahar Navodaya Vidyalaya · Mau, UP<br/>
  <sub>2017 · <b>89%</b></sub>
</td>
</tr>
</table>

---

## 📊 GitHub Analytics

<div align="center">
  <img width="60%" src="https://streak-stats.demolab.com?user=shubham221716&theme=tokyonight&hide_border=true&background=0d1117&ring=0e75b6&fire=0e75b6&currStreakLabel=0e75b6" alt="GitHub Streak" />
</div>

<br/>

<div align="center">

![Shubham's GitHub stats](https://github-readme-stats-eight-theta.vercel.app/api?username=shubham221716&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=0e75b6&icon_color=0e75b6&text_color=c9d1d9&rank_icon=github)
![Top Langs](https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=shubham221716&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=0e75b6&text_color=c9d1d9)

</div>

---

## 🏅 Achievements & Certifications

<div align="center">

| 🎯 | Achievement | Details |
|:---:|:---|:---|
| 💡 | **LeetCode Problem Solver** | 300+ problems · Arrays, Strings, Stacks, Queues, Linked Lists, Hashing |
| ⭐ | **CodeChef 2-Star Coder** | Max Rating 1443 · Silver Badge · 30+ programming contests |
| 🎓 | **AWS Academy Cloud Foundations** | Certified by AWS Academy |
| 🐳 | **Docker for DevOps** | Udemy Certified — Containerization, Compose, Deployment |
| 🏢 | **TCS — Cloud Engineer** | Production CI/CD, Terraform IaC, Docker on AWS |
| 🌐 | **Groveus Informatics** | Web Development · PHP · SEO Optimization |

</div>

---

## 🤝 Let's Connect & Collaborate

<div align="center">

<a href="https://linkedin.com/in/shubh45" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>&nbsp;
<a href="https://twitter.com/shubh_45_" target="_blank"><img src="https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white" alt="Twitter" /></a>&nbsp;
<a href="https://github.com/shubham221716" target="_blank"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" /></a>&nbsp;
<a href="https://www.leetcode.com/shubh_45_" target="_blank"><img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black" alt="LeetCode" /></a>&nbsp;
<a href="https://www.codechef.com/users/shubham221716" target="_blank"><img src="https://img.shields.io/badge/CodeChef-5B4638?style=for-the-badge&logo=codechef&logoColor=white" alt="CodeChef" /></a>&nbsp;
<a href="https://www.hackerrank.com/shubh_45_" target="_blank"><img src="https://img.shields.io/badge/HackerRank-2EC866?style=for-the-badge&logo=hackerrank&logoColor=white" alt="HackerRank" /></a>&nbsp;
<a href="https://shubh45.netlify.app/" target="_blank"><img src="https://img.shields.io/badge/Portfolio-FF5722?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Portfolio" /></a>&nbsp;
<a href="mailto:skg2002007@gmail.com"><img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>

</div>

---

<div align="center">

<!-- Snake Contribution Animation — auto-generated daily by .github/workflows/snake.yml -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/shubham221716/shubham221716/output/github-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/shubham221716/shubham221716/output/github-snake.svg" />
  <img alt="Snake animation" src="https://raw.githubusercontent.com/shubham221716/shubham221716/output/github-snake-dark.svg" />
</picture>

</div>

<!-- Footer Wave -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,50:1a1b27,100:0e75b6&height=120&section=footer" />
